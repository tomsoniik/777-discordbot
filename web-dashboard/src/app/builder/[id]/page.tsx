...
export default function BuilderCanvas({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [sidebarTab, setSidebarTab] = useState<'blocks' | 'color' | 'costs' | 'hotkeys'>('blocks');
  ...
  const itemCounts = useMemo(() => {
    ...
    placedItems.forEach(i => {
      map[i.itemId] = (map[i.itemId] || 0) + 1;
    });
    return map;
  }, [placedItems]);

  const filteredBuildItems = useMemo(() => {
    if (!searchQuery.trim()) return BUILD_ITEMS;
    const q = searchQuery.toLowerCase();
    return BUILD_ITEMS.filter(i => i.name.toLowerCase().includes(q) || i.id.toLowerCase().includes(q));
  }, [searchQuery]);
    ...
    <div className={styles.container}>
      ...
      <div className={`${styles.sidebar} ${isSidebarCollapsed ? styles.sidebarCollapsed : ''}`}>
        <div className={styles.tabsRow}>
          <button 
            className={`${styles.tabBtn} ${sidebarTab === 'blocks' ? styles.tabBtnActive : ''}`}
            onClick={() => setSidebarTab('blocks')}
          >
            🏗️ Klocki
          </button>
          <button 
            className={`${styles.tabBtn} ${sidebarTab === 'color' ? styles.tabBtnActive : ''}`}
            onClick={() => setSidebarTab('color')}
          >
            🎨 Kolory
          </button>
          <button 
            className={`${styles.tabBtn} ${sidebarTab === 'costs' ? styles.tabBtnActive : ''}`}
            onClick={() => setSidebarTab('costs')}
          >
            📊 Koszt
          </button>
          <button 
            className={`${styles.tabBtn} ${sidebarTab === 'hotkeys' ? styles.tabBtnActive : ''}`}
            onClick={() => setSidebarTab('hotkeys')}
          >
            ⌨️ Skróty
          </button>
        </div>
            ...
            <div className={styles.itemsGrid}>
              {filteredBuildItems.map(item => {
                ...
                return (
                  ...
                  >
                    {count > 0 && <span className={styles.itemCardBadge}>{count}</span>}
                    <div 
                      className={styles.itemCardThumb}
                      style={{ backgroundColor: (selectedColor && selectedColor !== 'clear') ? selectedColor : item.color }}
                    >
                      <img 
                        src={item.texture} 
                        alt="" 
                    ...
                    <span className={styles.itemCardName}>{t(item.id) || item.name}</span>
                  </div>
                );
              })}
            </div>
        ...
        {sidebarTab === 'color' && (
          <div>
            <div className={styles.sectionTitle}>Paleta Barw</div>
            <div className={styles.colorPaletteGrid}>
              {['', 'clear', '#ff4757', '#2ecc71', '#3498db', '#f1c40f', '#9b59b6', '#e67e22', '#10b981', '#38bdf8'].map(c => (
                <button
                  key={c}
                  className={`${styles.colorSwatch} ${selectedColor === c ? styles.colorSwatchActive : ''}`}
                  onClick={() => setSelectedColor(c)}
                  style={{ backgroundColor: c === 'clear' ? 'transparent' : (c || '#222') }}
                >
                  {!c && <span style={{ color: '#fff', fontSize: '10px' }}>OFF</span>}
                  {c === 'clear' && <span style={{ color: '#ff4757', fontSize: '14px', fontWeight: 'bold' }}>✕</span>}
                </button>
              ))}
            </div>

            <div className={styles.customColorPickerRow}>
              <span style={{ fontSize: '0.85rem' }}>Własny kolor:</span>
              <input 
                type="color" 
              ...
              />
              <span style={{ fontSize: '0.8rem', color: 'var(--accent-green)', fontWeight: 'bold' }}>
                {selectedColor || 'Domyślny'}
              </span>
            </div>
          </div>
        )}

        {/* Tab 3: Koszty Surowców */}
        {sidebarTab === 'costs' && (
          <div>
            <div className={styles.sectionTitle}>{t('builder_cost')}</div>
            <div className={styles.materialsList}>
              {Object.keys(totalMaterials).length === 0 ? (
                <div style={{ color: '#aaa', fontStyle: 'italic', fontSize: '0.85rem' }}>{t('builder_empty')}</div>
              ) : (
                Object.entries(totalMaterials).map(([mat, qty]) => (
                  <div key={mat} className={styles.materialRow}>
                    <span>{t(mat)}</span>
                    <span style={{ fontWeight: 'bold', color: '#10b981' }}>{qty}</span>
                  </div>
                ))
              )}
            </div>
        ...
        {/* Tab 4: Skróty klawiszowe */}
        {sidebarTab === 'hotkeys' && (
          <div className={styles.instructions}>
            <b>💡 Instrukcja i Skróty:</b><br />
            • <b>R / r:</b> Obrót aktywnego klocka<br />
            • <b>LMB (Lewy Przycisk):</b> Stawianie / Wybór<br />
            • <b>RMB (Prawy Przycisk):</b> Przesuwanie kamery 2D/3D<br />
            • <b>Delete:</b> Usuwanie zaznaczonego klocka<br />
            • <b>Ctrl + Z / Ctrl + Y:</b> Cofnij / Ponów<br />
            • <b>Shift + Click:</b> Zaznaczanie wielu klocków<br />
            • <b>Ctrl + C / Ctrl + V:</b> Kopiuj / Wklej
          </div>
        )}
      </div>
        ...
        <div className={styles.toolbar}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.6rem', alignItems: 'center' }}>
            <button 
              ...
              title="Cykl Dzień / Noc"
            >
              {isNightMode ? '🌙 Noc' : '☀️ Dzień'}
            </button>
            <button 
            ...
            >
              {is3DMode ? 'Wróć do 2D' : 'Widok 3D Sandbox'}
            </button>
            <button 
        ...
        >
          {is3DMode ? (
            <Builder3D 
              ...
              onSelectItem={(itemId, shiftKey) => {
                if (shiftKey) {
                  setSelectedItemIds(prev => prev.includes(itemId) ? prev.filter(i => i !== itemId) : [...prev, itemId]);
                } else {
              ...
              onDeleteItem={(itemId) => {
                commitAction(placedItems.filter(i => i.id !== itemId));
                setSelectedItemIds(prev => prev.filter(i => i !== itemId));
              }}
              onPaintItem={(itemId) => {
                if (selectedColor) {
