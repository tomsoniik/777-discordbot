    ...
    if (message.content === '!ticketsetup' && message.member?.permissions.has('Administrator')) {
        ...
        if ('send' in message.channel) {
            await message.channel.send({
                content:
                    'Kliknij poniższy przycisk, aby przejść do panelu rekrutacji i wypełnić formularz logując się przez Steam:',
                components: [row],
            });
        }
    }
        ...
        if (message.embeds.length > 0) {
            ...
            if (title.includes('raid') || desc.includes('destroyed') || title.includes('attack')) {
                ...
                const alertEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `Wyśledzono powiadomienie z serwera: **${foundMapName}**\n\n\`\`\`WBIJAĆ DO GRY BRONIĆ BAZY!\`\`\``,
                    )
                    .setColor('#FF0000')
