...
export const checkServersCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName('ip').setDescription('Adres IP serwera (np. 123.45.67.89)').setRequired(true),
        )
        .addIntegerOption((option) =>
            option.setName('port').setDescription('Główny port gry (np. 27015)').setRequired(true),
        ),
