    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        if (!ENV.STEAM_API_KEY) {
            await interaction.reply({
                content: 'Brak STEAM_API_KEY w konfiguracji bota!',
        ...
        if (!steamId) {
            await interaction.editReply(
                'Nie udało się rozwiązać SteamID z podanego wejścia. Podaj poprawny SteamID64 lub link do profilu publicznego.',
            );
            return;
        }
        ...
        try {
            ...
            // ignoruj błędy API steam podczas pobierania statusu profilu
        }

        const targetName =
            serverChoice === 'all'
                ? 'wszystkich zapisanych serwerach'
                : `serwerze ${PREDEFINED_SERVERS[serverChoice]?.displayName}`;

        if (existing && existing.isActive) {
            await prisma.trackedPlayer.update({
                where: { steamId },
                data: { targetServer: serverChoice, addedBy: interaction.user.id },
            });
            await interaction.editReply(
                `Zaktualizowano śledzenie gracza \`${steamId}\` na **${targetName}**!${privacyWarning}`,
            );
            return;
        }
        ...
        await interaction.editReply(
            `Rozpoczęto śledzenie SteamID: \`${steamId}\` na **${targetName}**!${privacyWarning}`,
        );
    },
};
